...
const props = defineProps({
  ...
  mode: { type: String, default: 'int', validator: (v) => ['int', 'float'].includes(v) },
});
...
watch(() => props.value, (val) => {
  ...
  if (resolved !== null) currentValue.value = resolved;
});

function resolveIncomingValue(val){
  if (val === null || val === undefined || val === '') return resolveInitialValue();
...
function onInput(event){
  ...
  if (!Number.isNaN(val)) currentValue.value = val;
}
