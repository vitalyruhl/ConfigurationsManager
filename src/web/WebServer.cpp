...
public:
  RuntimeMetaResponse(std::shared_ptr<const String> payload,
      ...
      : payload_(std::move(payload)), activeRequests_(activeRequests), configManager_(configManager), headerOffset_(0), payloadOffset_(0), sendStallLogged_(false) {
    _code = 200;
...
ConfigManagerWeb::ConfigManagerWeb(AsyncWebServer* webServer)
    : server(webServer), configManager(nullptr), initialized(false), routesDefined(false), serverStarted(false), embedWebUI(CM_EMBED_WEBUI ? true : false), customHTML(nullptr), customHTMLLen(0), settingsPassword("") // Empty by default - only protect if explicitly set
{
  if (!server) {
...
void ConfigManagerWeb::setupAPIRoutes() {
  ...
  server->on("/config_raw", HTTP_ANY, [this](AsyncWebServerRequest* request) {
  ...
  // Use AsyncCallbackJsonWebHandler to avoid edge cases with raw body accumulation.
  {
    auto* handler = new AsyncCallbackJsonWebHandler("/config/apply", [this](AsyncWebServerRequest* request, JsonVariant& json) {
      ...
      if (category.isEmpty() || key.isEmpty()) {
        ...
        AsyncWebServerResponse* response = request->beginResponse(400, "application/json", "{\"status\":\"error\",\"action\":\"apply\",\"reason\":\"missing_params\"}");
        enableCORS(response);
        request->send(response);
        return;
      }
      ...
      if (!json.is<JsonObject>()) {
        AsyncWebServerResponse* response = request->beginResponse(400, "application/json", "{\"status\":\"error\",\"action\":\"apply\",\"reason\":\"invalid_json\"}");
        enableCORS(response);
        request->send(response);
        return;
      }
      ...
      if (!obj.containsKey("value")) {
        AsyncWebServerResponse* response = request->beginResponse(400, "application/json", "{\"status\":\"error\",\"action\":\"apply\",\"reason\":\"missing_value\"}");
        enableCORS(response);
        request->send(response);
        return;
      }
      ...
      if (success) {
        ...
        AsyncWebServerResponse* response = request->beginResponse(400, "application/json", "{\"status\":\"error\",\"action\":\"apply\",\"reason\":\"update_failed\"}");
        enableCORS(response);
        request->send(response);
      }
    });
  ...
  // Use AsyncCallbackJsonWebHandler to avoid edge cases with chunked/unknown body sizes.
  {
    auto* handler = new AsyncCallbackJsonWebHandler("/config/save", [this](AsyncWebServerRequest* request, JsonVariant& json) {
      ...
      if (category.isEmpty() || key.isEmpty()) {
        ...
        AsyncWebServerResponse* response = request->beginResponse(400, "application/json", "{\"status\":\"error\",\"action\":\"save\",\"reason\":\"missing_params\"}");
        enableCORS(response);
        request->send(response);
        return;
      }
      ...
      if (!json.is<JsonObject>()) {
        AsyncWebServerResponse* response = request->beginResponse(400, "application/json", "{\"status\":\"error\",\"action\":\"save\",\"reason\":\"invalid_json\"}");
        enableCORS(response);
        request->send(response);
        return;
      }
      ...
      if (success) {
        ...
        AsyncWebServerResponse* response = request->beginResponse(400, "application/json", "{\"status\":\"error\",\"action\":\"save\",\"reason\":\"update_failed\"}");
        enableCORS(response);
        request->send(response);
      }
    });
...
String ConfigManagerWeb::issueSettingsAuthToken() {
  ...
  char buf[33];
  snprintf(buf, sizeof(buf), "%08lx%08lx%08lx%08lx", (unsigned long)r0, (unsigned long)r1, (unsigned long)r2, (unsigned long)r3);
