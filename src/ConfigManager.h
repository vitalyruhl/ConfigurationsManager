...
#define CONFIGMANAGER_VERSION "4.4.8" // Synced to library.json
...
public:
  bool hasError() const {
    return hasKeyLengthError;
  }
  const char* getError() const {
    return keyLengthErrorMsg.c_str();
  }
  ...
  BaseSetting(const char* key, const char* name, const char* category, SettingType type, bool showInWeb = true, bool isPassword = false, int sortOrder = 100, const char* categoryPretty = nullptr, const char* card = nullptr, const char* cardPretty = nullptr, int cardOrder = 100)
  ...
  const char* getCategoryPretty() const {
    return categoryPrettyName ? categoryPrettyName : category;
  }
  const char* getCard() const {
    return cardName;
  }
  const char* getCardPretty() const {
    return cardPrettyName ? cardPrettyName : cardName;
  }
  int getCardOrder() const {
    return cardOrder;
  }
...
public:
  ...
  public:
    ...
    LiveFieldBuilder checkbox(const char* key, const char* label, std::function<bool()> getter, std::function<void(bool)> setter) {
      RuntimeFieldMeta meta = createBaseMeta(key);
      meta.label = label ? label : meta.key;
    ...
    LiveFieldBuilder stateButton(const char* key, const char* label, std::function<bool()> getter, std::function<void(bool)> setter, bool initState = false, const char* onLabelText = nullptr, const char* offLabelText = nullptr) {
      RuntimeFieldMeta meta = createBaseMeta(key);
      meta.label = label ? label : meta.key;
    ...
    LiveFieldBuilder momentaryButton(const char* key, const char* label, std::function<bool()> getter, std::function<void(bool)> setter, const char* onLabelText = nullptr, const char* offLabelText = nullptr) {
      RuntimeFieldMeta meta = createBaseMeta(key);
      meta.label = label ? label : meta.key;
    ...
    LiveFieldBuilder intSlider(const char* key, const char* label, int minValue, int maxValue, int initValue, std::function<int()> getter, std::function<void(int)> setter, const char* unitText = nullptr) {
      RuntimeFieldMeta meta = createBaseMeta(key);
      meta.label = label ? label : meta.key;
    ...
    LiveFieldBuilder floatSlider(const char* key, const char* label, float minValue, float maxValue, float initValue, int precision, std::function<float()> getter, std::function<void(float)> setter, const char* unitText = nullptr) {
      RuntimeFieldMeta meta = createBaseMeta(key);
      meta.label = label ? label : meta.key;
    ...
    LiveFieldBuilder intInput(const char* key, const char* label, int minValue, int maxValue, int initValue, std::function<int()> getter, std::function<void(int)> setter, const char* unitText = nullptr) {
      RuntimeFieldMeta meta = createBaseMeta(key);
      meta.label = label ? label : meta.key;
    ...
    LiveFieldBuilder floatInput(const char* key, const char* label, float minValue, float maxValue, float initValue, int precision, std::function<float()> getter, std::function<void(float)> setter, const char* unitText = nullptr) {
      RuntimeFieldMeta meta = createBaseMeta(key);
      meta.label = label ? label : meta.key;
...
private:
  ...
  void sendSeverityMessage(const char* type,
    ...
    sendGUIMessage(type, title, message, details, actions, [this, messageId, contextBuilder](JsonObject& obj) {
...
public:
  ConfigManagerClass() {
    ...
    webManager.setCallbacks(
      ...
      [this]() { return runtimeManager.runtimeMetaJsonPayload(); }, // runtime meta JSON
      [this]() { reboot(); },                                       // reboot callback
      [this]() { for (const auto &entry : settings) entry->setDefault(); saveAll(); },                                                 // reset callback
  ...
  ConfigManagerRuntime& getRuntime() {
    return runtimeManager;
  }
  void setGuiLoggingEnabled(bool enabled) {
    guiLoggingEnabled = enabled;
  }
  bool isGuiLoggingEnabled() const {
    return guiLoggingEnabled;
  }
  ...
  BaseSetting* addSetting(BaseSetting* setting) {
    ...
    setting->setLogger([](const char* msg) { CM_CORE_LOG("%s", msg); });
  ...
  const char* getCustomCss() const {
    return customCss;
  }
  size_t getCustomCssLen() const {
    return customCss ? (customCssLen ? customCssLen : strlen(customCss)) : 0;
  }
  ...
  void defineRuntimeButton(const String& group, const String& key, const String& label, std::function<void()> onPress, const String& card = String(), int order = 100) {
    runtimeManager.defineRuntimeButton(group, key, label, onPress, card, order);
  }

  void defineRuntimeCheckbox(const String& group, const String& key, const String& label, std::function<bool()> getter, std::function<void(bool)> setter, const String& card = String(), int order = 100) {
    runtimeManager.defineRuntimeCheckbox(group, key, label, getter, setter, card, order);
  }

  void defineRuntimeStateButton(const String& group, const String& key, const String& label, std::function<bool()> getter, std::function<void(bool)> setter, bool initState = false, const String& card = String(), int order = 100, const String& onLabel = String(), const String& offLabel = String()) {
    runtimeManager.defineRuntimeStateButton(group, key, label, getter, setter, initState, card, order, onLabel, offLabel);
  }

  void defineRuntimeMomentaryButton(const String& group, const String& key, const String& label, std::function<bool()> getter, std::function<void(bool)> setter, const String& card = String(), int order = 100, const String& onLabel = String(), const String& offLabel = String()) {
    runtimeManager.defineRuntimeMomentaryButton(group, key, label, getter, setter, card, order, onLabel, offLabel);
  }

  void defineRuntimeIntSlider(const String& group, const String& key, const String& label, int minValue, int maxValue, int initValue, std::function<int()> getter, std::function<void(int)> setter, const String& unit = String(), const String& card = String(), int order = 100) {
    runtimeManager.defineRuntimeIntSlider(group, key, label, minValue, maxValue, initValue, getter, setter, unit, card, order);
  }

  void defineRuntimeFloatSlider(const String& group, const String& key, const String& label, float minValue, float maxValue, float initValue, int precision, std::function<float()> getter, std::function<void(float)> setter, const String& unit = String(), const String& card = String(), int order = 100) {
  ...
  void defineRuntimeIntValue(const String& group, const String& key, const String& label, int minValue, int maxValue, int initValue, std::function<int()> getter, std::function<void(int)> setter, const String& unit = String(), const String& card = String(), int order = 100) {
    runtimeManager.defineRuntimeIntValue(group, key, label, minValue, maxValue, initValue, getter, setter, unit, card, order);
  }

  void defineRuntimeFloatValue(const String& group, const String& key, const String& label, float minValue, float maxValue, float initValue, int precision, std::function<float()> getter, std::function<void(float)> setter, const String& unit = String(), const String& card = String(), int order = 100) {
  ...
  void enableBuiltinSystemProvider() {
    runtimeManager.enableBuiltinSystemProvider();
  }
  void updateLoopTiming() {
    runtimeManager.updateLoopTiming();
  }
  ...
  void setWebSocketInterval(uint32_t intervalMs) {
    wsInterval = clampWebSocketInterval_(intervalMs);
  }
  void setPushOnConnect(bool v) {
    pushOnConnect = v;
  }
  void setCustomLivePayloadBuilder(std::function<String()> fn) {
    customPayloadBuilder = fn;
  }
...
#if CM_ENABLE_WIFI
  ConfigManagerWiFi& getWiFiManager() {
    return wifiManager;
  }
#endif
  ConfigManagerWeb& getWebManager() {
    return webManager;
  }
  ConfigManagerOTA& getOTAManager() {
    return otaManager;
  }
  ConfigManagerRuntime& getRuntimeManager() {
    return runtimeManager;
  }
  ...
  void enableSmartRoaming(bool enable = true) {
    wifiManager.enableSmartRoaming(enable);
  }
  void setRoamingThreshold(int thresholdDbm = -75) {
    wifiManager.setRoamingThreshold(thresholdDbm);
  }
  void setRoamingCooldown(unsigned long cooldownSeconds = 120) {
    wifiManager.setRoamingCooldown(cooldownSeconds);
  }
  void setRoamingImprovement(int improvementDbm = 10) {
    wifiManager.setRoamingImprovement(improvementDbm);
  }
  bool isSmartRoamingEnabled() const {
    return wifiManager.isSmartRoamingEnabled();
  }

  // MAC Address Filtering and Priority - convenience methods
  void setAccessPointMacFilter(const String& macAddress) {
    wifiManager.setAccessPointMacFilter(macAddress);
  }
  void setAccessPointMacPriority(const String& macAddress) {
    wifiManager.setAccessPointMacPriority(macAddress);
  }
  void performStackReset() {
    wifiManager.performStackReset();
  }
  void clearMacFilter() {
    wifiManager.clearMacFilter();
  }
  void clearMacPriority() {
    wifiManager.clearMacPriority();
  }
  bool isMacFilterEnabled() const {
    return wifiManager.isMacFilterEnabled();
  }
  bool isMacPriorityEnabled() const {
    return wifiManager.isMacPriorityEnabled();
  }
  String getFilterMac() const {
    return wifiManager.getFilterMac();
  }
  String getPriorityMac() const {
    return wifiManager.getPriorityMac();
  }
#endif
