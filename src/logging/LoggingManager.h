...
public:
  ...
  public:
    ...
    void setPrefix(const char* prefix) {
      prefix_ = prefix ? String(prefix) : String();
    }
    ...
    void setTimestampFormat(const char* fmt) {
      timestampFormat_ = fmt ? String(fmt) : String();
    }
    const String& getTimestampFormat() const {
      return timestampFormat_;
    }

    void setMinIntervalMs(uint32_t ms) {
      minIntervalMs_ = ms;
    }
    ...
    void setFilter(std::function<bool(Level level, const char* tag, const char* message)> fn) {
      filter_ = std::move(fn);
    }
  ...
  public:
    ...
    void setRateLimitMs(uint32_t ms) {
      setMinIntervalMs(ms);
    }
  ...
  public:
    ...
    void setMaxQueue(size_t limit) {
      pendingLimit_ = limit;
    }
    void setMaxPerTick(size_t count) {
      maxPerTick_ = count;
    }
  ...
  void setGlobalLevel(Level level) {
    globalLevel_ = clampToBuildLevel_(level);
  }
  ...
  ScopedTag scopedTag(const char* tag) {
    return ScopedTag(this, tag);
  }
...
private:
  ...
  LoggingManager()
      : globalLevel_(buildLevel_()), defaultLevel_(buildLevel_()), verboseLevel_(buildLevel_()) {
  }
