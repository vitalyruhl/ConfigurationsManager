...
PinInfo GenericPinRules::getPinInfo(int /*pin*/) const {
  PinInfo info;
  info.exists = true;
  info.capabilities = static_cast<uint32_t>(PinCapability::DigitalIn) | static_cast<uint32_t>(PinCapability::DigitalOut) | static_cast<uint32_t>(PinCapability::AnalogIn) | static_cast<uint32_t>(PinCapability::DACOut) | static_cast<uint32_t>(PinCapability::PWMOut);
  info.constraints = 0;
  return info;
}
