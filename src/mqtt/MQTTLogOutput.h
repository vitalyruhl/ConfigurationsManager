...
public:
  ...
  explicit MQTTLogOutput(MQTTManager& mqtt, const char* logRoot = "log")
      : mqtt_(mqtt), logRoot_(logRoot && logRoot[0] ? String(logRoot) : String("log")) {
  }
  ...
  void setUnretainedEnabled(bool enabled) {
    unretainedEnabled_ = enabled;
  }

  void setRetainedLevels(bool info, bool warn, bool error) {
    retainedMask_ = 0;
    if (info)
      retainedMask_ |= RetainInfo;
    if (warn)
      retainedMask_ |= RetainWarn;
    if (error)
      retainedMask_ |= RetainError;
  }
  ...
  void setCustomRetainedEnabled(bool enabled) {
    customRetainedEnabled_ = enabled;
  }
