...
void setupGUI() {
  ...
  mqttOther.value("baseTopic", []() { return mqtt.getMqttBaseTopic(); })
    .label("Base Topic")
    .order(1);
  ...
  mqttReceived.value("test_energy_total_mwh", []() { return testEnergyTotalMWh; })
    .label("Test Energy Total")
    .unit("MWh")
    .precision(2)
    .order(4);

  mqttReceived.value("buttonState", []() { return buttonState; })
    .label("Button State")
    .order(5);
}
...
void Initial_logging() {
  ...
  // This shows how to add a dedicated log output without changing the core logger.
  {
    ...
    customLog->setFilter([](LL, const char* tag, const char*) { return tag && strncmp(tag, "Custom", 6) == 0; });
...
void sendAllMQTTLog(const unsigned long now, unsigned long& lastLoopLogMs) {
  if (now - lastLoopLogMs >= 10000) {
    ...
    lastLoopLogMs = now;
    lmg.logTag(LL::Info, "Loop", "wifi=%s mqtt=%s base=%s lastTopic=%s", WiFi.isConnected() ? "connected" : "disconnected", mqtt.isConnected() ? "connected" : "disconnected", mqtt.getMqttBaseTopic().c_str(), mqtt.getLastTopic().c_str());
