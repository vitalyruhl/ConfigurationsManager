...
void setupGUI() {
  ...
  limiter.value("invCalc", []() { return inverterCalculatedValue; })
  ...
  limiter.value("negPrice", []() { return negativePriceActive; })
    .label("Negative Price")
    .order(6);

  limiter.value("price", []() { return electricityPriceEurKwh; })
...
static void configureLimiterSettingsBehavior() {
  limiterSettings.smoothingSize.showIfFunc = []() { return !limiterSettings.usePidSmoothing.get(); };
...
static void normalizeNegativePriceSettings(NegativePriceSettingPreference preferred, bool persist, bool logCorrection) {
  ...
  if (logCorrection) {
    lmg.logTag(LL::Warn, "PRICE", "Corrected conflicting negative-price settings -> %s", winner == NegativePriceSettingPreference::SetZero ? "zero" : "min");
  }
  if (persist) {
    ConfigManager.saveAll();
  }
...
static void processRS485Tick() {
  ...
  if (negativePriceOverrideTarget >= 0) {
    ...
    if (usePidSmoothing) {
      lmg.logTag(LL::Trace, "PID", "inv=%d gridSigned=%d gridIn=%d gridOut=%d off=%d base=%d clamp=%d in=%d out=%d min=%d max=%d set=%d", currentInverterOutputW, signedGridPowerW, gridImportW, gridExportW, offset, pidBaseTarget, pidClampedBaseTarget, pidInput, inverterCalculatedValue, configuredMin, configuredMax, inverterSetValue);
    }
  } else {
